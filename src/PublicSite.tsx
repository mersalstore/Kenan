...
const services = [
  {
    ...
    body: "تصميم وتركيب شبكات الرش الآلي وأنظمة الغازات النظيفة (FM200, CO2) المعتمدة.",
    highlight: true,
  },
  {
    icon: ShieldCheck,
    title: "أنظمة إنذار الحريق",
    body: "لوحات ذكية وحساسات دقيقة مرتبطة بأنظمة الإخلاء الصوتي للكشف المبكر عن الحريق.",
    highlight: true,
  },
  {
    icon: Camera,
  ...
  {
    icon: Building2,
    title: "السلامة الصناعية والبيئية",
    body: "كشف تسرب الغازات، أنظمة الإطفاء للمطابخ والمصانع، والمراقبة البيئية للمنشآت.",
  },
  {
    icon: Wrench,
...
function LoginPage({
  ...
  onGoogleCredential: (credential: string) => void;
  onEmailLogin: (email: string, password: string) => void;
  onRegisterClient?: (name: string, phoneOrEmail: string) => void;
}) {
  ...
  const submitRegistration = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (regPhoneOrEmail.trim()) {
      if (onRegisterClient) {
        onRegisterClient(regName.trim() || "عميل جديد", regPhoneOrEmail.trim());
      } else {
