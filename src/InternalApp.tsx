...
export function InternalApp({ user, onLogout, onOpenSite }: InternalAppProps) {
  ...
  const roleFilteredProjects = useMemo(() => {
    ...
    const userEngId = String((user as any).backendId || user.id || matchedStaff?.id || matchedStaff?.backendId || "").trim();
  ...
  }, [projects, isSiteEngineer, user.name, user.email, (user as any).backendId, user.id, staff, assignments]);
