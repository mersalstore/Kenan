...
export type SiteSettings = {
  ...
  contractGeneralTerms?: string;
  contractFines?: string;
...
export type StaffAccount = {
  id: number;
  backendId?: string; // معرف المستخدم الحقيقي في قاعدة البيانات
  name: string;
  email: string;
  password: string;
  role: string;
...
// ===== تقرير اليوم الموحد لمهندس الموقع (يُخزَّن فعليًا عبر الـ API، وليس محليًا) =====
export type DailyReportSystemType = "FIRE_ALARM" | "FIRE_FIGHTING" | "VENTILATION";

export type DailyReportSystemEntry = {
  id: string;
  reportId: string;
  systemType: DailyReportSystemType;
...
// ===== طلبات التوريد واستلام البضاعة (تُخزَّن عبر الـ API) =====
export type SupplyOrderStatus = "PENDING" | "PARTIAL" | "RECEIVED";
...
export type WorkTeam = {
  id: number | string;
  name: string;
  subcontractorId: number | string | null; // null = فريق داخلي
  teamLead: string;
  trade: string;
};
...
export type ProjectAssignment = {
  ...
  subcontractorId: number | string | null;
  roleOnSite: string;
  startDate: string;
  endDate: string;
};
